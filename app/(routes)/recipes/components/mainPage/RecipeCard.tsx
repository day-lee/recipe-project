...
return(
    ...
    <Link href={`/recipes/${recipe.public_id}`}>
        ...
        className='border-slate-600 shadow-md bg-white sm:w-[230px] h-[350px] sm:h-[330px] transition-all duration-300 hover:shadow-lg hover:-translate-y-1'>
            {recipe.img_link ? 
            <Image className='w-full h-[230px] sm:h-[210px] object-cover' priority={true} src={recipe.img_link} alt={recipe.recipe_name} width={210} height={210} 
                                sizes="(max-width: 668px) 80vw, 80vw" /> 
            :
            <Image className="w-full h-[230px] sm:h-[210px] object-cover" src={fallbackImg} alt='fallbackImg' width={210} height={210} sizes="(max-width: 668px) 80vw, 80vw"/>
            }
